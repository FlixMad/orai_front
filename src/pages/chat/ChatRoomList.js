...
const ChatRoomList = ({ onChatRoomCreated }) => {
    ...
    }, [currentUser.id]); // chatRooms 제거, 사용자 변경시에만 재연결

    // 채팅방 구독 관리를 위한 별도 useEffect
    useEffect(() => {
        if (!stompClient || !chatRooms.length) return;

        // 채팅방 메시지 구독
        chatRooms.forEach((room) => {
            stompClient.subscribe(`/sub/${room.chatRoomId}/chat`, (message) => {
        ...
        stompClient.subscribe(`/queue/queue`, (notification) => {
            const data = JSON.parse(notification.body);
            alert(data.message);
            fetchChatRooms(); // 채팅방 목록 새로고침
        });
    }, [stompClient, chatRooms, currentChatId]);
            ...
            if (basicResponse.status === 200 && unreadResponse.status === 200) {
                ...
                const mergedRooms = basicResponse.data.map((basicRoom) => {
                    const unreadInfo = unreadResponse.data.find(
                        (unreadRoom) =>
                            unreadRoom.chatRoomId === basicRoom.chatRoomId
                    );
                    return {
                        ...basicRoom,
                    ...
                    <ModalContent onClick={(e) => e.stopPropagation()}>
                        ...
                        <ImageInputWrapper>
                            ...
                            <ImageInput
                                ...
                                accept="image/*"
                                onChange={(e) =>
                                    setEditRoomImage(e.target.files[0])
                                }
                                id="createRoomImage"
                            />
                            <ImageLabel htmlFor="createRoomImage">
                        ...
                        />
                        <ButtonGroup>
                            <UpdateButton onClick={handleUpdateRoom}>
                                수정
                            </UpdateButton>
                            <DeleteButton
            ...
            {isDeleteConfirmOpen && (
                <Modal onClick={() => setIsDeleteConfirmOpen(false)}>
                    <ModalContent onClick={(e) => e.stopPropagation()}>
                        ...
                        <ModalText>
                            채팅방을 삭제하면 채팅방 구독자들이 자동으로
                            채팅방에서 퇴장하게 되며 모든 데이터가 사라집니다.
                            정말로 이 채팅방을 삭제하시겠습니까?
                        </ModalText>
                        <ButtonGroup>
